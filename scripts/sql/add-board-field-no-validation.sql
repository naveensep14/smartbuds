-- Disable question validation temporarily and add board field
-- This script removes the validation trigger and adds the board field

-- Step 1: Drop the question validation trigger if it exists
DROP TRIGGER IF EXISTS validate_questions ON tests;

-- Step 2: Drop the question validation function if it exists
DROP FUNCTION IF EXISTS validate_questions_structure();

-- Step 3: Add the board column to the tests table
ALTER TABLE tests 
ADD COLUMN IF NOT EXISTS board TEXT DEFAULT 'ICSE';

-- Step 4: Update existing tests with appropriate board values
...
-- Step 5: Add constraints for the board field
ALTER TABLE tests 
...
-- Step 7: Add index for board + subject + grade filtering
CREATE INDEX IF NOT EXISTS idx_tests_board_subject_grade ON tests(board, subject, grade);

-- Step 8: Update the unique constraint to include board
...
-- Step 9: Create a function to validate board values
...
-- Step 10: Create trigger to validate board values
...
-- Step 11: Create a view for board statistics
...
-- Step 13: Test the board field functionality
DO $$
DECLARE
  test_id UUID;
BEGIN
  ...
  RAISE NOTICE '⚠️  Question validation has been disabled for now';
END $$;
