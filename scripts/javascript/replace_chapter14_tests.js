...
async function replaceChapter14Tests() {
  try {
    console.log('🔄 Replacing Chapter 14 Tests...');
    console.log('==================================');
    ...
    // Read the corrected Chapter 14 tests
    const fs = require('fs');
    const path = require('path');
    
    const testsFile = path.join(__dirname, 'tmp', 'tests', 'chapter_14_tests.json');
    
    if (!fs.existsSync(testsFile)) {
      console.error('❌ Chapter 14 tests file not found:', testsFile);
      return;
    }

    const testsData = JSON.parse(fs.readFileSync(testsFile, 'utf8'));
    console.log(`📖 Found ${testsData.length} corrected Chapter 14 tests to import`);
    ...
    if (successCount > 0) {
      console.log('\n🎉 Chapter 14 tests replaced successfully!');
      console.log('🔍 The tests now have proper money-related questions.');
      console.log('📋 No more duplicate questions between the two tests!');
    }
