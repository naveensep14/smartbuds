...
`;
  ...
  async processPDF(pdfBuffer, subject, grade, board, customPrompt = '') {
    try {
      ...
      // Check PDF size (Gemini has a 20MB limit)
      const pdfSizeMB = pdfBuffer.length / (1024 * 1024);
      console.log(`📄 PDF size: ${pdfSizeMB.toFixed(2)} MB`);
      
      if (pdfSizeMB > 20) {
        throw new Error(`PDF file is too large (${pdfSizeMB.toFixed(2)} MB). Gemini API supports files up to 20 MB.`);
      }
      
      if (pdfSizeMB > 10) {
        console.log('⚠️ Large PDF detected. Processing may take longer and could timeout.');
      }
