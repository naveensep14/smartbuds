...
class PDFProcessor {
  constructor() {
    ...
    // Initialize Together AI as open-source fallback
    if (process.env.TOGETHER_API_KEY) {
      this.together = new Together({
        apiKey: process.env.TOGETHER_API_KEY,
      });
      this.fallbackModel = 'meta-llama/Llama-3.1-70B-Instruct-Turbo';
    } else {
      console.log('⚠️ Together API key not found - open-source fallback disabled');
      this.together = null;
    }
  }
...
`;
  ...
  // Fallback method using Together AI (Llama) when Gemini fails
  async generateQuestionsWithTogether(concept, subject, grade, pdfText, customPrompt = '', numQuestions = 10) {
    try {
      if (!this.together) {
        throw new Error('Together AI client not initialized - API key missing');
      }

      console.log('🦙 [TOGETHER FALLBACK] Starting question generation with Llama...');
      console.log('🦙 [TOGETHER FALLBACK] Concept:', concept);
      console.log('🦙 [TOGETHER FALLBACK] Subject:', subject);
      console.log('🦙 [TOGETHER FALLBACK] Grade:', grade);
      console.log('🦙 [TOGETHER FALLBACK] PDF Text Length:', pdfText ? pdfText.length : 0);
...
Do not include any text before or after the JSON array.`;
      ...
      console.log('🦙 [TOGETHER FALLBACK] Calling Together AI API...');
      ...
      console.log('🦙 [TOGETHER FALLBACK] Raw response:', response.choices[0].message.content);
      ...
      console.log('🦙 [TOGETHER FALLBACK] Cleaned response:', aiResponse);
      ...
      console.log(`🦙 [TOGETHER FALLBACK] Generated ${questions.length} questions successfully`);
      
      return questions.map((q, index) => ({
        id: `together_${Date.now()}_${index}`,
        question: q.question,
        options: q.options,
      ...
      console.error('🦙 [TOGETHER FALLBACK] Error:', error.message);
      throw new Error(`Together AI fallback failed: ${error.message}`);
    }
  }
  ...
  async generateQuestionsForConceptWithFallback(concept, subject, grade, pdfBuffer, customPrompt = '', numQuestions = 10) {
    try {
      ...
      console.log('🤖 [FALLBACK] Gemini failed, trying Together AI fallback...');
      console.log('🤖 [FALLBACK] Gemini error:', geminiError.message);
      
      try {
        // Extract text from PDF buffer for Together AI (simplified approach)
        const pdfText = `PDF content for ${concept} - ${subject} - ${grade}`;
        return await this.generateQuestionsWithTogether(concept, subject, grade, pdfText, customPrompt, numQuestions);
      } catch (togetherError) {
        console.log('🤖 [FALLBACK] Together AI failed, using template fallback...');
        console.log('🤖 [FALLBACK] Together AI error:', togetherError.message);
  ...
  async generateQuestionsForConceptWithFileURIFallback(concept, subject, grade, pdfBuffer, customPrompt = '', geminiFileName, numQuestions = 10) {
    try {
      ...
      console.log('🤖 [FILE URI FALLBACK] Gemini File URI failed, trying Together AI fallback...');
      ...
      try {
        ...
        console.log('🤖 [FILE URI FALLBACK] Together AI failed, using template fallback...');
        console.log('🤖 [FILE URI FALLBACK] Together AI error:', togetherError.message);
  ...
  // Fallback method for concept identification using Together AI
  async identifyConceptsWithTogether(subject, grade) {
    try {
      if (!this.together) {
        throw new Error('Together AI client not initialized - API key missing');
      }

      console.log('🦙 [TOGETHER CONCEPTS] Starting concept identification with Llama...');
      console.log('🦙 [TOGETHER CONCEPTS] Subject:', subject);
      console.log('🦙 [TOGETHER CONCEPTS] Grade:', grade);
...
Do not include any explanatory text, just the JSON array.`;
      ...
      console.log('🦙 [TOGETHER CONCEPTS] Calling Together AI API...');
      ...
      console.log('🦙 [TOGETHER CONCEPTS] Raw response:', response.choices[0].message.content);
      ...
      console.log('🦙 [TOGETHER CONCEPTS] Cleaned response:', aiResponse);
      ...
      console.log(`🦙 [TOGETHER CONCEPTS] Generated ${concepts.length} concepts successfully`);
      
      return concepts;

    } catch (error) {
      console.error('🦙 [TOGETHER CONCEPTS] Error:', error.message);
      throw new Error(`Together AI concept identification failed: ${error.message}`);
    }
  }

  // Template-based question generation (no AI required)
  generateTemplateQuestions(concept, subject, grade, numQuestions = 10) {
    console.log('📝 [TEMPLATE FALLBACK] Generating template-based questions...');
    ...
    for (let i = 0; i < numQuestions; i++) {
      ...
      const question = {
        id: `template_${Date.now()}_${questionNumber}`,
        question: `What is the main concept related to ${concept} in ${subject} for ${grade} students?`,
        options: [
          `Option A: Basic understanding of ${concept}`,
          `Option B: Advanced application of ${concept}`,
          `Option C: Historical context of ${concept}`,
          `Option D: Future implications of ${concept}`
        ],
        correctAnswer: 0, // Option A is always correct for template questions
        explanation: `This question tests basic understanding of ${concept} concepts appropriate for ${grade} level ${subject} students.`,
        image: undefined
      };
    ...
    console.log(`📝 [TEMPLATE FALLBACK] Generated ${questions.length} template questions`);
    return questions;
  }

  // Template-based concept identification (no AI required)
  getTemplateConcepts(subject, grade) {
    console.log('📝 [TEMPLATE FALLBACK] Using template-based concepts...');
    
    const conceptTemplates = {
      'Mathematics': {
        '1st Grade': ['Numbers 1-20', 'Basic Addition', 'Shapes and Patterns'],
        '2nd Grade': ['Addition and Subtraction', 'Place Value', 'Measurement'],
        '3rd Grade': ['Multiplication', 'Fractions', 'Geometry'],
        '4th Grade': ['Division', 'Decimals', 'Area and Perimeter'],
        '5th Grade': ['Fractions Operations', 'Algebra Basics', 'Data Analysis'],
        '6th Grade': ['Ratios and Proportions', 'Integers', 'Coordinate Plane'],
        '7th Grade': ['Linear Equations', 'Probability', 'Transformations'],
        '8th Grade': ['Quadratic Equations', 'Functions', 'Statistics'],
        '9th Grade': ['Polynomials', 'Trigonometry', 'Coordinate Geometry'],
        '10th Grade': ['Calculus Basics', 'Sequences', 'Complex Numbers']
      },
      'Science': {
        '1st Grade': ['Living Things', 'Weather', 'Plants and Animals'],
        '2nd Grade': ['Matter States', 'Simple Machines', 'Life Cycles'],
        '3rd Grade': ['Forces and Motion', 'Habitats', 'Earth Materials'],
        '4th Grade': ['Energy', 'Ecosystems', 'Rocks and Minerals'],
        '5th Grade': ['Cells', 'Solar System', 'Chemical Changes'],
        '6th Grade': ['Genetics', 'Atoms', 'Climate'],
        '7th Grade': ['Evolution', 'Periodic Table', 'Weather Patterns'],
        '8th Grade': ['Human Body Systems', 'Chemical Reactions', 'Earth Structure'],
        '9th Grade': ['Molecular Biology', 'Physics Laws', 'Environmental Science'],
        '10th Grade': ['Organic Chemistry', 'Quantum Physics', 'Ecology']
      },
      'English': {
        '1st Grade': ['Phonics', 'Reading Comprehension', 'Basic Grammar'],
        '2nd Grade': ['Vocabulary', 'Sentence Structure', 'Story Elements'],
        '3rd Grade': ['Parts of Speech', 'Writing Skills', 'Literary Devices'],
        '4th Grade': ['Paragraph Writing', 'Poetry', 'Research Skills'],
        '5th Grade': ['Essay Writing', 'Figurative Language', 'Critical Reading'],
        '6th Grade': ['Advanced Grammar', 'Creative Writing', 'Literary Analysis'],
        '7th Grade': ['Persuasive Writing', 'Drama', 'Media Literacy'],
        '8th Grade': ['Research Papers', 'Shakespeare', 'Rhetorical Devices'],
        '9th Grade': ['Literary Criticism', 'Advanced Composition', 'Public Speaking'],
        '10th Grade': ['Comparative Literature', 'Technical Writing', 'Debate Skills']
      }
    };

    const subjectConcepts = conceptTemplates[subject] || conceptTemplates['Mathematics'];
    const gradeConcepts = subjectConcepts[grade] || subjectConcepts['5th Grade'];
    
    console.log(`📝 [TEMPLATE FALLBACK] Generated concepts for ${grade} ${subject}:`, gradeConcepts);
    return gradeConcepts;
  }
  ...
  async identifyConceptsWithFallback(pdfBuffer, subject, grade) {
    try {
      ...
      console.log('🤖 [CONCEPTS FALLBACK] Gemini failed, trying Together AI fallback...');
      console.log('🤖 [CONCEPTS FALLBACK] Gemini error:', geminiError.message);
      
      try {
        return await this.identifyConceptsWithTogether(subject, grade);
      } catch (togetherError) {
        console.log('🤖 [CONCEPTS FALLBACK] Together AI failed, using template fallback...');
        console.log('🤖 [CONCEPTS FALLBACK] Together AI error:', togetherError.message);
        
        // Use template-based concepts as final fallback
        return this.getTemplateConcepts(subject, grade);
      }
    }
  }
}
