...
Do not include any text before or after the JSON array.`;
      ...
      // Check if response contains error messages or is not JSON
      if (aiResponse.includes('Request') || aiResponse.includes('Error') || aiResponse.includes('Failed') || 
          ...
          aiResponse.includes('API') || aiResponse.includes('invalid') || aiResponse.includes('unauthorized') ||
          aiResponse.includes('Entity Too Large') || aiResponse.includes('413') || aiResponse.includes('too large') ||
...
`;
  ...
  async processPDF(pdfBuffer, subject, grade, board, customPrompt = '', chapter = 1) {
    try {
      ...
      if (pdfSizeMB > 10) {
        console.log('⚠️ Large PDF detected. Using File API for processing.');
      }
      ...
      console.error('PDF Size:', pdfBuffer ? `${(pdfBuffer.length / (1024 * 1024)).toFixed(2)} MB` : 'Unknown');
      console.error('Subject:', subject);
      console.error('Grade:', grade);
      console.error('Board:', board);
      console.error('Chapter:', chapter);
      console.error('Custom Prompt:', customPrompt ? customPrompt.substring(0, 100) : 'None');
