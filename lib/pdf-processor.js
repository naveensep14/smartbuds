...
`;
  ...
  async processPDFWithFileURI(pdfBuffer, subject, grade, board, type = 'coursework', customPrompt = '', chapter = 1, geminiFileName, numTests = 5, questionsPerTest = 10) {
    try {
      ...
      for (let testIndex = 0; testIndex < numTests; testIndex++) {
        ...
        const questions = await this.generateQuestionsForConceptWithFileURI(concept, subject, grade, pdfBuffer, customPrompt, geminiFileName, questionsPerTest);
  ...
  // Fallback method using Ollama/Llama when Gemini fails
  async generateQuestionsWithLlama(concept, subject, grade, pdfText, customPrompt = '', numQuestions = 10) {
    try {
      console.log('🦙 [LLAMA FALLBACK] Starting question generation with Llama...');
      console.log('🦙 [LLAMA FALLBACK] Concept:', concept);
      console.log('🦙 [LLAMA FALLBACK] Subject:', subject);
      console.log('🦙 [LLAMA FALLBACK] Grade:', grade);
      console.log('🦙 [LLAMA FALLBACK] PDF Text Length:', pdfText ? pdfText.length : 0);
...
Do not include any text before or after the JSON array.`;
      ...
      console.log('🦙 [LLAMA FALLBACK] Calling Ollama API...');
      ...
      console.log('🦙 [LLAMA FALLBACK] Raw response:', response.response);
      ...
      console.log('🦙 [LLAMA FALLBACK] Cleaned response:', aiResponse);
      ...
      console.log(`🦙 [LLAMA FALLBACK] Generated ${questions.length} questions successfully`);
      ...
      throw new Error(`Llama fallback failed: ${error.message}`);
    }
  }
  ...
  async generateQuestionsForConceptWithFallback(concept, subject, grade, pdfBuffer, customPrompt = '', numQuestions = 10) {
    try {
      ...
      console.log('🤖 [FALLBACK] Gemini failed, trying Llama fallback...');
      console.log('🤖 [FALLBACK] Gemini error:', geminiError.message);
      
      try {
        // Extract text from PDF buffer for Llama (simplified approach)
        const pdfText = `PDF content for ${concept} - ${subject} - ${grade}`;
        return await this.generateQuestionsWithLlama(concept, subject, grade, pdfText, customPrompt, numQuestions);
      } catch (llamaError) {
        console.error('🦙 [FALLBACK] Both Gemini and Llama failed');
        throw new Error(`Both AI models failed. Gemini: ${geminiError.message}, Llama: ${llamaError.message}`);
      }
    }
  }
}
