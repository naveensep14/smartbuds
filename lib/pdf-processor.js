...
class PDFProcessor {
  ...
  async generateQuestionsForConceptWithFileURI(concept, subject, grade, pdfBuffer, customPrompt = '', geminiFileName) {
    try {
      ...
      const prompt = customPrompt || `Generate exactly 10 multiple-choice questions about ${concept} for ${grade} students studying ${subject}. 
...
`;
  ...
  async processPDFWithFileURI(pdfBuffer, subject, grade, board, customPrompt = '', chapter = 1, geminiFileName) {
    try {
      ...
      for (const concept of concepts) {
        ...
        const questions = await this.generateQuestionsForConceptWithFileURI(concept, subject, grade, pdfBuffer, customPrompt, geminiFileName);
