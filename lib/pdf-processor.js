...
class PDFProcessor {
  constructor() {
    ...
    // No AI fallback - use templates only
    console.log('📝 Template fallback enabled for when Gemini fails');
  }
...
`;
  ...
  async generateQuestionsForConceptWithFallback(concept, subject, grade, pdfBuffer, customPrompt = '', numQuestions = 10) {
    try {
      ...
      console.log('🤖 [FALLBACK] Gemini failed, using template fallback...');
      console.log('🤖 [FALLBACK] Gemini error:', geminiError.message);
      
      // Use template-based questions as fallback
  ...
  async generateQuestionsForConceptWithFileURIFallback(concept, subject, grade, pdfBuffer, customPrompt = '', geminiFileName, numQuestions = 10) {
    try {
      ...
      console.log('🤖 [FILE URI FALLBACK] Gemini File URI failed, using template fallback...');
  ...
  async identifyConceptsWithFallback(pdfBuffer, subject, grade) {
    try {
      ...
      console.log('🤖 [CONCEPTS FALLBACK] Gemini failed, using template fallback...');
      console.log('🤖 [CONCEPTS FALLBACK] Gemini error:', geminiError.message);
      
      // Use template-based concepts as fallback
