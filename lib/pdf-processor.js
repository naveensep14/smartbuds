...
`;
  ...
  async processPDFWithFileURI(pdfBuffer, subject, grade, board, customPrompt = '', chapter = 1, geminiFileName, numTests = 5, questionsPerTest = 10) {
    try {
  ...
  async processPDF(pdfBuffer, subject, grade, board, customPrompt = '', chapter = 1, numTests = 5, questionsPerTest = 10) {
    try {
      ...
      for (let testIndex = 0; testIndex < numTests; testIndex++) {
        ...
        const questions = await this.generateQuestionsForConcept(concept, subject, grade, pdfBuffer, customPrompt, questionsPerTest);
        console.log(`✅ [VERCEL LOG] Generated ${questions.length} questions for ${concept} (Test ${testIndex + 1})`);
